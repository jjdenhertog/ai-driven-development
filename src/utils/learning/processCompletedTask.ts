...
import { getUserChangesSafe } from '../git/getUserChangesSafe';
...
export async function processCompletedTask(task: Task): Promise<void> {
    ...
    try {
        ...
        const userChanges = getUserChangesSafe(task.branch, task.id);
        ...
        if (!userChanges || userChanges.fileChanges.length === 0) {
            ...
            execSync(`git commit -m "chore: archive task ${task.id} - no user changes needed (AI-generated)"`, { stdio: 'pipe' });
        ...
        execSync('git add -A', { stdio: 'pipe' });
        execSync(`git commit -m "chore: archive task ${task.id} after learning from user changes (AI-generated)"`, { stdio: 'pipe' });
